...
public class SUSActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_susactivity);
        ...
        List<SUSResultData> resultList = dbHelper.getSUSResultsForUser(userId);

        if (!resultList.isEmpty()) {
            for (SUSResultData result : resultList) {
                addTableRow(resultTable, result);
            }
        } else {
            addEmptyTableRow(resultTable, "Πατήστε το κουμπί παρακάτω για να προσθέσετε αποτελέσματα SUS");
        }

        addResultButton.setOnClickListener(v -> {
            Intent intent = new Intent(SUSActivity.this, SUSActivityResults.class);
            startActivity(intent);
        });

        homeButton.setOnClickListener(v -> {
            Intent intent = new Intent(SUSActivity.this, ProfileActivity.class);
    ...
    private void addTableRow(TableLayout tableLayout, SUSResultData result) {
        TableRow row = new TableRow(this);

        TextView scoreTextView = new TextView(this);
        scoreTextView.setText(String.valueOf(result.getScore()));
        setTableRowAttributes(scoreTextView);
        row.addView(scoreTextView);

        TextView multipliedScoreTextView = new TextView(this);
        multipliedScoreTextView.setText(String.valueOf(result.getMultipliedScore()));
        setTableRowAttributes(multipliedScoreTextView);
        row.addView(multipliedScoreTextView);
